...
import Ast
  ( Expression (Annotation, Application, Function, If, Let, LiteralExpression, Variable, annotationExpression, annotationType, applicationArguments, applicationFunction, functionBody, functionParameters, ifCondition, ifElse, ifThen, letDefinition, letName),
    ParserExpression,
...
type InferenceExpression = Expression InferenceExpressionVar InferenceTypeVar
...
data TranslationError = UnboundedVariable ParserExpressionVariable deriving (Show)
...
freshExpressionVar :: (State TranslationState :> es) => Eff es InferenceExpressionVar
...
freshTypeVar :: (State TranslationState :> es) => Eff es InferenceTypeVar
freshTypeVar = do
  ...
  case expr of
    ...
    Application
      { applicationFunction = appFunction,
        ...
        pure Application {applicationFunction = newFunction, applicationArguments = newArguments}
    ...
    Annotation {annotationExpression = ae, annotationType = at} -> do
      ...
      pure Annotation {annotationExpression = newExpression, annotationType = newType}
