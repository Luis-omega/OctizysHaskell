  ...
  describe "between" $ do
    it "parses correctly surrounded by open and close" $ do
      let open = text "("
          close = text ")"
          p = text "abc"
      runParser (between @() open close p) "(abc)" `shouldBe` Right "abc"

    it "fails if the open parser doesn't match" $ do
      let open = text "["
          close = text "]"
          p = text "abc"
      runParser (between @() open close p) "(abc)" `shouldSatisfy` isLeft
